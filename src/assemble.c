...
static vm_opKind ic_getVmCast(ic_vmProgram *program, ic_op op, corto_type t, ic_vmType typeKind, ic_vmOperand storage, ic_vmOperand op1) {
    ...
    {
        corto_assert(
          result != CORTO_VM_STOP,
          "no cast-instruction found from type '%s' to '%s'",
...
{\
    if (t->kind == CORTO_PRIMITIVE) {\
        ...
        default:{\
            corto_assert(0,\
              "conditional operation " #op " works only for numeric primitives (got %s)",\
...
{\
    if (t->kind == CORTO_PRIMITIVE) {\
        ...
        default:{\
            corto_assert(\
              0,\
              "conditional operation " #op " works only for INTEGER, UINTEGER and FLOAT primitives (got %s)",\
              corto_fullpath(NULL, t));\
            break;\
        }\
        }\
...
static void ic_getVmOp(ic_vmProgram *program, ic_op op) {
    ...
    case ic_push:
        if (op->s1Any) {
            op2 = (ic_node)ic_address__create(NULL, NULL, (corto_word)ic_valueType(op->s1));
    ...
    case ic_deinit: {
        op1 = op->s1;
        op2 = (ic_node)ic_object__create(NULL, NULL, ic_storage(op->s1)->type);
    ...
    case ic_set:
        if (ic_storage(op->s2)->type->kind == CORTO_ITERATOR) {
            op1 = op->s2;
            op2 = op->s3;
            op3 = (ic_node)ic_object__create(NULL, NULL, ic_storage(op->s3)->type);
            opDeref1 = op->s2Deref;
            opDeref2 = op->s3Deref;
